...
const types = {
  ...
  '.webm': 'video/webm', '.pdf': 'application/pdf', '.woff2': 'font/woff2'
};
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (parts.some(part => part.startsWith('.') || part.startsWith('_') || /[\\:\0]/.test(part))) {
      return reply(404, 'Not found.');
    }
    const relative = parts.length ? parts.join('/') : 'index.html';
    const extension = path.extname(relative).toLowerCase();
    if (!types[extension] || (parts.length > 1 && !['assets', 'img', 'fonts'].includes(parts[0]))) {
      return reply(404, 'Not found.');
    }
    const file = await realpath(path.resolve(root, relative));
    if (!insideRoot(file)) return reply(404, 'Not found.');
    ...
    if (!res.headersSent) reply(error instanceof URIError ? 400 : 404, 'Not found.');
    else res.destroy();
  }
});
