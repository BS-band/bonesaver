...
import { web3formsAccessKey } from "./web3forms-config.mjs";
...
function cta(title = "Vaše akce může znít takhle.") {
  return `<section class="section cta-section"><div class="container cta-inner"><div><p class="eyebrow">BoneSaver na vaší akci</p><h2>${title}</h2><p class="contract-inline-link"><a href="/kontakt.html#dokumenty">Smlouva, stage plán a repertoár ke stažení</a></p></div>${link("/kontakt.html#poptavka", "Ověřit volný termín", "button")}</div></section>`;
}
function contractDownloads() {
  const base = '/assets/documents/bonesaver-smlouva-hudebni-produkce-vzor';
  return `<section class="section downloads-section" id="dokumenty"><div class="container split-content"><div>${sectionTitle('Dokumenty ke stažení', 'Podklady pro vaši akci', 'Vzor smlouvy, stage plán pro zvukaře a plesový repertoár jako podklad pro OSA. Vše přehledně na jednom místě.')}<p class="small muted">Termín, rozsah vystoupení, cenu a ostatní podmínky vždy doplníme a potvrdíme společně. Stažení ani vyplnění vzoru samo o sobě nerezervuje termín.</p><p class="small muted">Vzor smlouvy obsahuje zálohu 50 % do 14 dnů od podpisu a storno 50 % při dobrovolném zrušení pořadatelem; podrobnosti a výjimky najdete přímo ve smlouvě.</p></div><div class="download-list"><a class="download-document" href="${base}.pdf" download><span class="document-format" aria-hidden="true">PDF</span><span><strong>Stáhnout vzor smlouvy v PDF</strong><span>Smlouva s přílohou · pro přečtení a tisk</span></span>${arrow}</a><a class="download-document" href="${base}.docx" download><span class="document-format" aria-hidden="true">DOCX</span><span><strong>Stáhnout vzor smlouvy ve Wordu</strong><span>Upravitelný dokument · pro doplnění údajů</span></span>${arrow}</a><p class="small muted">Pracovní vzor smlouvy · verze 24. 9. 2026. Před podpisem doplnit údaje a odsouhlasit podmínky.</p><a class="download-document" href="/assets/documents/bonesaver-stage-plan.pdf" download><span class="document-format" aria-hidden="true">PDF</span><span><strong>Stáhnout stage plán v PDF</strong><span>BoneSaver · rozmístění kapely a technické zapojení</span></span>${arrow}</a><a class="download-document" href="/assets/documents/bonesaver-repertoar-pro-plesy-osa-2026.pdf" download><span class="document-format" aria-hidden="true">PDF</span><span><strong>Repertoár pro plesy / OSA v PDF</strong><span>BoneSaver 2026 · 60 skladeb · podklad pro pořadatele</span></span>${arrow}</a><p class="small muted">Repertoár slouží jako podklad pro pořadatele, nikoli jako vyplněné hlášení pro OSA. Pro konkrétní akci je potřeba uvést skutečně odehrané skladby.</p></div></div></section>`;
}
function home() {
  const preview = [
    ["Pohoda", "Kabát", "Rock"],
...
function privacy() {
  return `${pageHero("Soukromí", "Vaše údaje<br><span>při domluvě hraní.</span>", "Co se děje při odeslání poptávky, přepnutí vzhledu a přehrávání videí.")}<section class="section"><div class="container prose"><h2>Kdo vyřizuje poptávku</h2><p>Ing. Miroslav Bečička, IČO 69126887, Do Polí 1054, 530 06 Pardubice. Pro dotazy k osobním údajům použijte <a href="mailto:${site.email}">${site.email}</a>.</p><h2>Odeslání poptávky</h2><p>Po stisknutí tlačítka „Odeslat nezávaznou poptávku“ odešle prohlížeč jméno, e-mail, případný telefon a údaje o akci službě <a href="https://web3forms.com/" target="_blank" rel="noopener noreferrer">Web3Forms ↗</a>. Ta zprávu předá na e-mailovou adresu přiřazenou k formuláři. Při otevření stránky se formulář této službě nepřipojuje. Údaje formuláře neukládáme do úložiště prohlížeče.</p><h2>Domluva vystoupení</h2><p>Pokud nám zprávu odešlete, použijeme její obsah a kontaktní údaje pro vyřízení vašeho dotazu, přípravu nabídky a případnou dohodu o vystoupení. Tyto kroky před uzavřením smlouvy provádíme na vaši žádost. Zprávy uchováváme po dobu potřebnou k vyřízení domluvy; při objednávce se na související doklady vztahují zákonné povinnosti. Samotná poptávka vás nepřihlašuje k marketingovým zprávám.</p><h2>Videa a externí služby</h2><p>Náhledy videí i písma se načítají přímo z tohoto webu. Teprve po kliknutí na přehrání se připojí přehrávač YouTube v režimu rozšířené ochrany soukromí. YouTube přitom může zpracovat například vaši IP adresu a informace o zařízení. <a href="https://policies.google.com/privacy?hl=cs" target="_blank" rel="noopener noreferrer">Zásady ochrany soukromí společnosti Google ↗</a></p><h2>Uložený vzhled</h2><p>Po přepnutí světlého nebo tmavého motivu si web může uložit pouze vaši volbu vzhledu do localStorage. Nepoužívá ji ke sledování. Tato verze neobsahuje analytické ani reklamní měřicí skripty.</p><h2>Vaše práva</h2><p>Můžete se na nás obrátit se žádostí o přístup k údajům, jejich opravu, výmaz nebo omezení zpracování. Podle konkrétní situace můžete mít také právo na přenositelnost nebo vznést námitku. Pokud máte za to, že s údaji není nakládáno správně, můžete kontaktovat Úřad pro ochranu osobních údajů.</p><p class="small muted">Informace odpovídají formuláři s odesíláním přes Web3Forms. Aktualizováno 24. 9. 2026.</p></div></section>`;
}
const pages = [
  {
    file: "index.html",
  ...
  },
  {
    ...
    description:
      "Informace o odesílání poptávky, ochraně osobních údajů, uloženém vzhledu a přehrávání videí na webu BoneSaver.",
    body: privacy(),
  },
  {
    file: "404.html",
