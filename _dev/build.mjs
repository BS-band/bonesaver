...
function cta(title = "Vaše akce může znít takhle.") {
  return `<section class="section cta-section"><div class="container cta-inner"><div><p class="eyebrow">BoneSaver na vaší akci</p><h2>${title}</h2><p class="contract-inline-link"><a href="/kontakt.html#dokumenty">Předem si můžete projít vzor smlouvy</a></p></div>${link("/kontakt.html#poptavka", "Ověřit volný termín", "button")}</div></section>`;
}
function contractDownloads() {
  const base = '/assets/documents/bonesaver-smlouva-hudebni-produkce-vzor';
  return `<section class="section downloads-section" id="dokumenty"><div class="container split-content"><div>${sectionTitle('Dokumenty ke stažení', 'Smlouva a podmínky hraní', 'Projděte si pracovní vzor smlouvy včetně technické přílohy. Termín, rozsah vystoupení, cenu a ostatní podmínky vždy doplníme a potvrdíme společně.')}<p class="small muted">Stažení ani vyplnění vzoru samo o sobě nerezervuje termín. Vzor obsahuje zálohu 50 % do 14 dnů od podpisu a storno 50 % při dobrovolném zrušení pořadatelem; podrobnosti a výjimky najdete přímo ve smlouvě.</p></div><div class="download-list"><a class="download-document" href="${base}.pdf" download><span class="document-format" aria-hidden="true">PDF</span><span><strong>Stáhnout vzor smlouvy v PDF</strong><span>Smlouva s přílohou · pro přečtení a tisk</span></span>${arrow}</a><a class="download-document" href="${base}.docx" download><span class="document-format" aria-hidden="true">DOCX</span><span><strong>Stáhnout vzor smlouvy ve Wordu</strong><span>Upravitelný dokument · pro doplnění údajů</span></span>${arrow}</a><p class="small muted">Pracovní vzor · verze 24. 9. 2026. Před podpisem doplnit údaje a odsouhlasit podmínky.</p></div></div></section>`;
}
function home() {
  const preview = [
    ["Pohoda", "Kabát", "Rock"],
